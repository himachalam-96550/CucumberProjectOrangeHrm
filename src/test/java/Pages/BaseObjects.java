...
public class BaseObjects extends DriverFactory{
    ...
    public static ButtonHelper butonhelper = new ButtonHelper();
	 ...
	 public static BrowserHelper  browserHelper = new BrowserHelper();
		...
		/*
		 * public static ReadTestData.Root readTestData() { ObjectMapper objectMapper =
		 * new ObjectMapper(); ReadTestData.Root testData = null;
		 * 
		 * File file = new
		 * File(BaseObjects.class.getClassLoader().getResource("LoginTestData.json").
		 * getFile());
		 * 
		 * try { testData = objectMapper.readValue(file, ReadTestData.Root.class); }
		 * catch (IOException e) { e.printStackTrace(); } return testData; }
		 */
